...
const __delete = (self, toDelete) => {
  ...
  if (toDeleteLen) {
    ...
    _.remove(self._data, o => ids.indexOf(o._id.toString()) !== -1);
  }
...
const _buildCursor = (data, filter, options) => {
  ...
  const cursor = mingoQuery.find(data);
  if (properOptions.limit) {
    cursor.limit(properOptions.limit);
  }
  if (properOptions.sort) {
    cursor.sort(properOptions.sort);
  }
  if (properOptions.skip) {
    cursor.skip(properOptions.skip);
  }
  return cursor;
};
...
class Collection {
  ...
  aggregate(pipeline, options, callback) {
    const self = this;
    return wrapCall(() => {
      const agg = aggregator(this.db, pipeline, options);
      return new Promise((resolve, reject) => {
        agg.run(self._data, null, (err, col) => {
          if (err) {
            reject(err);
          } else {
            // mongoose 5 expect a cursor
            col.toArray = (cb) => { cb(null, col); };
            resolve(col);
          }
        });
      });
  ...
  find(query, fields, cb) {
    const properCallback = arguments[arguments.length - 1];
    const cursor = new Cursor(this._data, query);
    if (typeof properCallback !== 'function') {
      return cursor;
    }
    process.nextTick(() => {
      properCallback(null, cursor);
    });
  }
  ...
  findOneAndDelete(filter, options, callback) {
    return wrapCall(() => {
      ...
      if (index !== -1) {
        const value = this._data.splice(index, 1);
        return {
          value: value[0],
          ok: 1,
          lastErrorObject: null,
  ...
  findOneAndReplace(filter, replacement, options, callback) {
    return wrapCall(() => {
      ...
      const returnOriginal = _.get(options, 'returnOriginal', true);
      const index = _indexByQuery(this._data, filter, options);
      const replaced = this._data[index];
      this._data[index] = replacement;
      return {
        value: returnOriginal ? replaced : replacement,
        ok: 1,
        lastErrorObject: null,
      };
    }, arguments);
  ...
  update(selector, document, options, callback) {
    ...
    return this.updateOne(selector, document, options).then(onResolve);
  }
  ...
  updateOne(filter, update, options, callback) {
    ...
    return wrapCall(() => {
      ...
      if (doc) {
        ...
        const updateExecutor = new UpdateExecutor(update, true);
        const newDocument = updateExecutor.applyUpdate(q);
        this._data.push(newDocument);
        return {
          result: {
            n: 1,
